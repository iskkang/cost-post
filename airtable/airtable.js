...
const fetchRecords = async (table, filterFormula) => {
  try {
    ...
    const records = await base(table).select({
      filterByFormula: filterFormula,  // 필터 공식
      view: "Grid view"  // 특정 뷰를 사용하여 조회
    ...
    console.error('Airtable API error:', error.message);  // 에러 메시지만 출력하도록 수정
    if (error.response) {
      console.error('Airtable API response error:', error.response.data);
    }
